...
describe('toOPPRPlayer', () => {
  it('should transform user with rating and IFPA data', () => {
    ...
    expect(player.ratings.glicko?.value).toBe(1850); // Matchplay rating preferred by default
    expect(player.ranking).toBe(250); // IFPA ranking
    expect(player.isRated).toBe(true); // 120 events >= 5
    expect(
      (player.ratings.glicko as { value: number; ratingDeviation: number }).ratingDeviation
    ).toBe(45);
  ...
  it('should prefer IFPA rating when configured', () => {
    ...
    expect(player.ratings.glicko?.value).toBe(1720); // IFPA rating
    expect(
      (player.ratings.glicko as { value: number; ratingDeviation: number }).ratingDeviation
    ).toBe(350); // Default RD since IFPA doesn't provide it
  });
  ...
  it('should handle user without rating data', () => {
    ...
    expect(player.ratings.glicko?.value).toBe(1500); // Default
  ...
  it('should use custom default values', () => {
    ...
    expect(player.ratings.glicko?.value).toBe(1400);
